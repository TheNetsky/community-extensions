...
var source = (() => {
  ...
  // src/Hentai2Read/Hentai2ReadHelper.ts
  var entities = require_lib2();
  var decodeHTMLEntity = (str) => {
    return entities.decodeHTML(str);
  };
  var getFormBody = (query) => {
    let formBody = "cmd_wpm_wgt_mng_sch_sbm=Search&txt_wpm_wgt_mng_sch_nme=&cmd_wpm_pag_mng_sch_sbm=&txt_wpm_pag_mng_sch_nme=" + (query.title ?? "") + "&txt_wpm_pag_mng_sch_ats=" + (query.parameters?.["artist"] ?? "") + "&txt_wpm_pag_mng_sch_chr=" + (query.parameters?.["character"] ?? "") + "&txt_wpm_pag_mng_sch_rls_yer=" + (query.parameters?.["release-year"] ?? "");
  ...
  // src/Hentai2Read/Hentai2ReadTags.ts
  var categories = [];
  var tags = [];
  var getTagId = async ($, label) => {
    await populateTags($);
    const category = categories.find((tag2) => tag2.label === label);
    if (category) {
      return category.id;
    }
  ...
  // src/Hentai2Read/Hentai2ReadParser.ts
  var nextSearchPageUrl = "";
  var parseMangaDetails = async ($, mangaId) => {
  ...
  var parseHomePageSectionMangas = ($) => {
    const section_Array = [];
  ...
  var parseSearchResults = ($, metadata) => {
  ...
  var parseViewMoreItems = ($, metadata) => {
    const page = metadata?.page ?? 1;
    const mangas = [];
    for (const manga of $(".col-xs-6.col-sm-4.col-md-3.col-xl-2", ".row.book-grid").toArray()) {
      const image = $("div > div > picture > img", manga).attr("src") ?? "";
  ...
  // src/Hentai2Read/Hentai2Read.ts
  var DOMAIN = "https://hentai2read.com";
  var Hentai2ReadInfo = {
    version: "1.0.1",
    name: "Hentai2Read",
    icon: "icon.png",
