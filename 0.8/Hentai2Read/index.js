...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Hentai2Read = exports.Hentai2ReadInfo = exports.DOMAIN = void 0;
...
class Hentai2Read {
    ...
    getMangaShareUrl(mangaId) { return `${exports.DOMAIN}/${mangaId}`; }
    async getMangaDetails(mangaId) {
        ...
        return (0, Hentai2ReadParser_1.parseMangaDetails)($, mangaId);
    }
    async getChapters(mangaId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, Hentai2ReadParser_1.parseChapters)($, mangaId);
    }
    async getChapterDetails(mangaId, chapterId) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, Hentai2ReadParser_1.parseChapterDetails)($, mangaId, chapterId);
    }
    async getHomePageSections(sectionCallback) {
        ...
        for (const section of sections) {
            ...
            const response = await this.requestManager.schedule(request, 1);
            const $ = this.cheerio.load(response.data);
            section.items = (0, Hentai2ReadParser_1.parseHomePageSectionMangas)($);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, Hentai2ReadParser_1.parseViewMoreItems)($, metadata);
    }
    async getSearchResults(query, metadata) {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, Hentai2ReadParser_1.parseSearchResults)($, metadata);
    }
    async getSearchFields() {
        return (0, Hentai2ReadParser_1.parseSearchFields)();
    }
    async getSearchTags() {
        const request = App.createRequest({
        ...
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        return (0, Hentai2ReadParser_1.parseSearchTags)($);
    }
...
},{"./Hentai2ReadHelper":71,"./Hentai2ReadParser":72,"./Hentai2ReadTags":73,"@paperback/types":61}],71:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.parseDate = exports.isLastPage = exports.getFormBody = exports.decodeHTMLEntity = void 0;
const entities = require("entities");
const decodeHTMLEntity = (str) => {
    return entities.decodeHTML(str);
};
exports.decodeHTMLEntity = decodeHTMLEntity;
const getFormBody = (query) => {
...
const parseMangaDetails = async ($, mangaId) => {
    ...
    for (const tag of $('div.col-xs-12.col-sm-7 > ul > li:nth-child(n+11):nth-child(-n+12) > a').toArray()) {
        ...
        const id = await (0, Hentai2ReadTags_1.getTagId)($, label);
        if (!id || !label)
            continue;
        arrayTags.push({ id: id, label: label });
    }
    const tagSections = [App.createTagSection({ id: '0', label: 'content', tags: arrayTags.map(x => App.createTag(x)) })];
...
exports.parseHomePageSectionMangas = parseHomePageSectionMangas;
...
const getTagId = async ($, label) => {
    await (0, exports.populateTags)($);
    const category = exports.categories.find(tag => tag.label === label);
    if (category) {
        return category.id;
    }
    const tag = exports.tags.find(tagVar => tagVar.label === label);
    if (tag) {
        return tag.id;
    }
    return null;
...
const getTags = async ($, tagSelector) => {
    const tags = [];
...
const populateTags = async ($) => {
    if (exports.categories.length != 0 && exports.tags.length != 0) {
        return;
    }
    exports.categories = await getTags($, '#tab-category');
    exports.tags = await getTags($, '#tab-tag');
};
exports.populateTags = populateTags;

},{"./Hentai2ReadHelper":71}]},{},[70])(70)
});
