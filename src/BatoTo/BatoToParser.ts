...
export const parseMangaDetails = ($: CheerioStatic, mangaId: string): SourceManga => {
    ...
    const author = $('div.attr-item b:contains(authors) span').text().trim()
    const artist = $('div.attr-item b:contains(artists) span').text().trim()
...
export const parseChapterList = ($: CheerioStatic): Chapter[] => {
    ...
    for (const chapter of $('div.episode-list div.main .item').toArray()) {
        ...
        const language = $('div.attr-item b:contains(translated language) span').text().trim() ?? 'Unknown'
